...
const FrmPartyList = () => {
 ...
 const loadCorporations = async () => {
  try {
    ...
    if (ulbId && corpList.length > 0) {
      const selectedCorpId = ulbId.toString();

      const matchedCorp = corpList.find(
        (corp) =>
          corp.NUM_CORPORATION_ID?.toString() === selectedCorpId
      );

      if (matchedCorp) {
        setSelectedCorp(selectedCorpId);
        await loadParties(selectedCorpId);
      }
    }
  } catch (err) {
