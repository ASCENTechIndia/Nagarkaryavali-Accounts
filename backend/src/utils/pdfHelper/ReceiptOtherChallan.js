...
const ReceiptOtherChallanPDFHelper = async ({ reportData, filters, corporationName, corporationLogo }) => {
    try {
        if (!reportData.length) throw new Error("No data");

        console.log("reportData", reportData);

        const templatePath = path.resolve(__dirname, "../../templates/ReceiptOtherChallan.html");
        ...
        const groupedRows = Object.values(
            reportData.reduce((acc, row) => {
                ...
                acc[code].amount += Number(row.AMOUNT || row.amount || 0);
