...
const generateMutationReportPDF = async (mutationData, mutationTypeName, documents) => {
  let browser;
  ...
  try {
    ...
    const templatePath = path.resolve(
      __dirname,
      "../../templates/transfer-application-report.html"
    );
    ...
    const formattedData = {
      ...
      proparea: mutationData.proparea ? Number(mutationData.proparea).toFixed(2) : "0.00",
      transferfee: mutationData.transferfee ? Number(mutationData.transferfee).toLocaleString("en-IN") : "0.00",
      chaintransferfee: mutationData.chaintransferfee ? Number(mutationData.chaintransferfee).toLocaleString("en-IN") : "0.00",
